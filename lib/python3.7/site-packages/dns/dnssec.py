...
def key_id(key, origin=None):
    ...
    total = sum((rdata[2 * i] << 8) + \
                rdata[2 * i + 1] for i in range(len(rdata) // 2))
    if len(rdata) % 2 != 0:
...
def _find_candidate_keys(keys, rrsig):
    ...
    if isinstance(value, dns.node.Node):
        ...
        rdataset = value
    return [rdata for rdata in rdataset if rdata.algorithm == rrsig.algorithm and \
...
def _validate(rrset, rrsigset, keys, origin=None, now=None):
    ...
    *now* is an ``int``, the time to use when validating the signatures,
    ...
    rrname = rrset[0] if isinstance(rrset, tuple) else rrset.name
