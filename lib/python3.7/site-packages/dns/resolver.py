...
class NXDOMAIN(dns.exception.DNSException):
    ...
    def _check_kwargs(self, qnames, responses=None):
        ...
        return dict(qnames=qnames, responses=responses)
...
class Resolver(object):
    ...
    def use_tsig(self, keyring, keyname=None,
        ...
        *algorithm*, a ``dns.name.Name``, the TSIG algorithm to use.
        ...
        self.keyname = list(self.keyring.keys())[0] if keyname is None else keyname
...
def _getaddrinfo(host=None, service=None, family=socket.AF_UNSPEC, socktype=0,
    ...
    try:
        ...
        if host is None:
            ...
            parts = host.split('%')
            ahost = parts[0] if len(parts) == 2 else host
    ...
    try:
        ...
        port = 0 if service is None else int(service)
    except Exception:
    ...
    if socktype == 0:
        ...
        socktypes = [socktype]
    cname = canonical_name if flags & socket.AI_CANONNAME != 0 else ''
    if family in [socket.AF_INET6, socket.AF_UNSPEC]:
        for addr in v6addrs:
